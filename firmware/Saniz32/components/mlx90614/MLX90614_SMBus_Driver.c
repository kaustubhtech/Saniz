...
void MLX90614_SMBusInit(uint8_t sda_gpio, uint8_t scl_gpio, int freq, int port){   
    sda = sda_gpio;
    scl = scl_gpio;
    i2c_speed = freq;           //addition by Kaustubh
    i2c_port = port;            // addition by Kaustubh
    
    i2c_config_t MLX90614_config = {
        .mode = I2C_MODE_MASTER,       /*!< I2C mode */
        .sda_io_num = sda_gpio,        /*!< GPIO number for I2C sda signal */
        .sda_pullup_en = GPIO_PULLUP_DISABLE,//GPIO_PULLUP_ENABLE,  /*!< Internal GPIO pull mode for I2C sda signal*/
        .scl_io_num = scl_gpio,       /*!< GPIO number for I2C scl signal */
        .scl_pullup_en = GPIO_PULLUP_DISABLE, //GPIO_PULLUP_ENABLE,  /*!< Internal GPIO pull mode for I2C scl signal*/
   ...
   ESP_ERROR_CHECK_WITHOUT_ABORT(i2c_param_config(port, &MLX90614_config));
   ESP_ERROR_CHECK_WITHOUT_ABORT(i2c_driver_install(port, I2C_MODE_MASTER, I2C_MASTER_RX_BUF_DISABLE, I2C_MASTER_TX_BUF_DISABLE, 0));
}
...
int MLX90614_SMBusRead(uint8_t slaveAddr, uint8_t reg_addr, uint16_t *data){
    ...
    ESP_ERROR_CHECK_WITHOUT_ABORT(i2c_driver_delete(i2c_port));

    MLX90614_SMBusInit(sda, scl, i2c_speed, i2c_port);

    i2c_cmd_handle_t cmd = i2c_cmd_link_create();
    ESP_ERROR_CHECK_WITHOUT_ABORT(i2c_master_start(cmd));
    ESP_ERROR_CHECK_WITHOUT_ABORT(i2c_master_write_byte(cmd, chip_addr | WRITE_BIT, ACK_CHECK_EN));
    ESP_ERROR_CHECK_WITHOUT_ABORT(i2c_master_write_byte(cmd, data_addr, ACK_CHECK_EN));
    ESP_ERROR_CHECK_WITHOUT_ABORT(i2c_master_start(cmd));
    chip_addr = chip_addr | 0x01;
    ESP_ERROR_CHECK_WITHOUT_ABORT(i2c_master_write_byte(cmd, chip_addr | READ_BIT, ACK_CHECK_EN));
    ESP_ERROR_CHECK_WITHOUT_ABORT(i2c_master_read(cmd, recived_data, 2, ACK_VAL));
    ESP_ERROR_CHECK_WITHOUT_ABORT(i2c_master_read_byte(cmd, recived_data + 2, NACK_VAL));       
...
int MLX90614_SMBusWrite(uint8_t slaveAddr, uint8_t writeAddress, uint16_t data)
{
    ...
    ESP_ERROR_CHECK_WITHOUT_ABORT(i2c_master_write_byte(cmd, chip_addr | WRITE_BIT, ACK_CHECK_EN));
    for (int i = 0; i < 3; i++) {
        ESP_ERROR_CHECK_WITHOUT_ABORT(i2c_master_write_byte(cmd, send_data[i], ACK_CHECK_EN));
    }
    ESP_ERROR_CHECK_WITHOUT_ABORT(i2c_master_stop(cmd));
    esp_err_t ret = i2c_master_cmd_begin(I2C_NUM_0, cmd, 1000 / portTICK_RATE_MS);
    i2c_cmd_link_delete(cmd);
